...
| Trades page | Section 8 trade_log schema — every column + all 19 status values + their meaning; Section 10 "Trade history default filter" for why it defaults to Executed only |
...
- **The Trades page and a stock's own trade table both default to "Executed only" (SUCCESS + FAILED)**, not every status — client request, since the raw signal log is dominated by skip reasons. `StatusCombobox` is the one shared component both pages use for this (three tiers: Executed only / All statuses / a specific status); don't reintroduce a separate flat `<Select>` for status filtering. The backend accepts a comma-separated `status` list (`GET /trades?status=SUCCESS,FAILED`) for this — see Section 10 "Trade history default filter". "Executed only" is the baseline, not an active filter: `hasActiveFilters` must not count it, and "Clear filters" must return to it, not to "All statuses".
