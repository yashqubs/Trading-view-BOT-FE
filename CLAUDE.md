...
- **Any component rendering a formatted timestamp must call `useTimezone()`**, even if it discards the result. The top bar has a UK/India/USA display-timezone picker; `formatDate`/`formatDateTime` read the active zone from a module variable in `lib/timezone.ts` rather than taking it as an argument, so a component without the context subscription silently keeps showing the old clock after a zone change. Zone is display-only — the wire stays UTC. Use `formatCalendarDate` (not `formatDateTime`) for `YYYY-MM-DD` values like chart buckets and date-range filters: those are calendar dates, not instants, and shifting them slides a day's totals onto the wrong label.
- **Size is displayed with IG's sign** via `formatSignedQuantity(size, direction)` — shorts negative, longs positive. IG's API reports size unsigned with the direction in a separate field, so the sign is derived from direction; don't render `size` raw, and don't re-negate an already-negative value.
- **`isClosingTrade` is only meaningful on `SUCCESS`/`FAILED` rows.** The `/trades` "Open / Close" column reads it (direction hasn't answered open-vs-close since short selling), but the DB column is non-nullable with a `false` default and `TradeService.logSkip` never sets it — so every *skipped* signal reads `false` regardless of what was held. Render "—" for anything that isn't `SUCCESS` or `FAILED`, or the table claims orders that were never placed.
- The `/positions` **Opened** column comes from IG's own `createdDateUTC` (`OpenPosition.openedAt`, ISO 8601 UTC or null), not from trade history — positions opened outside the bot still have one. Null means IG reported no usable timestamp: render "—", never a fallback to now. Sorting puts nulls last in both directions.
